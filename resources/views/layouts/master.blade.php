...
<!-- Fonts -->
    ...
    <style type="text/css">
        ...
        .bigdrop.select2-container .select2-results {min-height: 200px;}
        .bigdrop .select2-results {min-height: 200px;}
        .bigdrop .select2-choices {min-height: 150px; min-height: 150px; overflow-y: auto;}
        ...
        <div class="collapse navbar-collapse navbar-collapse-toolbar" id="site-navbar-collapse">
            ...
            <ul class="nav navbar-toolbar navbar-right navbar-toolbar-right">
                        ...
                        <select class="form-control " id="cpny" onchange="setcpny()">
                            ...
                            @foreach($companies as $company)
                                <option value="{{$company->id}}" {{$company->
                                    id==session('company_id')?'selected':''}}>{{$company->name}}
                                </option>
                            @endforeach
              ...
              <span class="avatar avatar-online">
                <img
                    src="{{ file_exists(public_path('uploads/avatar'.Auth::user()->image))?asset('uploads/avatar'.Auth::user()->image):(Auth::user()->sex=='M'?asset('global/portraits/male-user.png'):asset('global/portraits/female-user.png'))}}"
                    alt="...">
                <i></i>
              </span>
                    </a>
                    ...
                    <li class="nav-item dropdown">
                        ...
                        <div class="dropdown-menu" role="menu">
                            ...
                            @if(Auth::user()->role->permissions->contains('constant', 'payroll_setting'))
                                <a class="dropdown-item" href="{{ url('payrollsettings') }}" role="menuitem"> Payroll
                                    Settings</a>
                                <div class="dropdown-divider"></div>
                            @endif
                            @if(Auth::user()->role->permissions->contains('constant', 'workflows'))
                                <a class="dropdown-item" href="{{ url('workflows') }}" role="menuitem"> Workflow
                                    Settings</a>
                        </div>
                        @endif
                    </li>
                ...
                <li class="nav-item dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right dropdown-menu-media" role="menu">
                        <div class="dropdown-menu-header">
                            ...
                            @if(count(Auth::user()->unreadNotifications))
                                <span
                                    class="tag tag-round tag-danger">New {{count(Auth::user()->unreadNotifications)}}</span>
                            @endif
                        </div>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $("#global_menu_search").select2({ width: '97%', border:'none', dropdownCssClass: "bigdrop",placeholder:'Enter a menu item to search' });


        $(".site-menubar-body a").each(function(index,element){
            if(index==0){
                $('#global_menu_search').append('<option selected value="0">-Enter a menu item to search-</option>');
            }

            $('#global_menu_search').append(`<option value="${element.href==='javascript:void(0)' ? '{{url('/')}}' : element.href}">${element.text}</option>`);
    ...
    $(function () {
        $(document).on('click', '.notificationbutton', function (event) {
            ...
            href = $(this).attr('href');
            id = $(this).attr('id');
            $.get('{{url('userprofile/clear_notification?notification_id=')}}' + id, function (data, status, xhr) {
...
<script type="text/javascript">
    ...
    function generateCourseLoop(list){
        ...
        list.forEach(function(v,k){
            ...
            $el.find('#access-link').attr('href','http://elearning.thehcmatrix.com/hcm_onboard/index.php?endpoint=autologin&username={{ Auth::user()->emp_num }}&course=' + v.id);
            $parentEl.append($el);
        });
    }
